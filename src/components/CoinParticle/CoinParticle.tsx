...
const CoinParticle: React.FC<CoinParticleProps> = ({
  ...
  // Animation constants
  const ANIMATION_DURATION = 7500; // 7.5 seconds
  ...
  useEffect(() => {
    ...
    const startDelay = setTimeout(() => {
      ...
      const animate = (currentTime: number) => {
        ...
        const newX = startPosition.x + velocity.x * elapsed;
        const newY = startPosition.y + velocity.y * elapsed + 0.5 * GRAVITY * elapsed * elapsed;
  ...
  }, [startPosition, velocity, onComplete, delay, denomination]);
